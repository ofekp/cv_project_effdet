...
def rescale(matrix, bbox_tensor, target_dim, pad_color=0, interpolation=Image.NEAREST):
    mode = None
    ...
    if target_dim:
        ...
        new_bbox_tensor = torch.mul(bbox_tensor, ratio)
        new_bbox_tensor[:, 0] += ((target_dim - new_size[0]) // 2)
        new_bbox_tensor[:, 1] += ((target_dim - new_size[1]) // 2)
        new_bbox_tensor[:, 2] += ((target_dim - new_size[0]) // 2)
        new_bbox_tensor[:, 3] += ((target_dim - new_size[1]) // 2)
    else:
        new_im = matrix_img
        new_bbox_tensor = bbox_tensor

    trans = transforms.ToTensor()
    if isinstance(matrix, Image.Image):
        return trans(new_im), new_bbox_tensor
    else:
        return trans(new_im) * 255, new_bbox_tensor  # TODO(ofekp): note that masks will get this
